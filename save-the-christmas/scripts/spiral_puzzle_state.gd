...
func update_physics(delta: float) -> void:
	for ring in rings:
		if ring != null and not ring.is_merged:
...
func set_ring_velocity(ring_index: int, velocity: float) -> void:
	...
	if ring != null and not ring.is_merged:
		...
		ring.angular_velocity = clamp(velocity,
...
func use_hint() -> int:
	...
	# Find all rings that are not at correct angle
	for i in range(rings.size()):
		var ring = rings[i]
		if ring != null and not ring.is_merged and not ring.is_angle_correct():
